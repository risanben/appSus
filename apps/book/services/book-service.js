...
function addReview(bookToUpdate) {
...
function saveBookFromSearch(book) {
  console.log('book:', book)
  let newBook = {
    id: book.id,
    title: book.volumeInfo.title,
    authors: ['puki ben david'],
    publishedDate: book.volumeInfo.publishedDate,
    description:"placerat nisi sodales suscipit tellus tincidunt mauris elit sit luctus interdum ad dictum platea vehicula conubia fermentum habitasse congue suspendisse",
    thumbnail: book.volumeInfo.imageLinks.smallThumbnail,
    listPrice:{amount:200, currencyCode:'ILS', isOnSale: false,
  reviews:[]}
  }
  return newBook
}
...
function save(book) {
  if (book.subtitle) return _update(book)
...
function _update(bookToUpdate) {
  // MAKING SURE THE PRICE IS IN THE RIGHT PLACE 
  bookToUpdate.listPrice.amount = bookToUpdate.price
  console.log('bookToUpdate:', bookToUpdate)
  let books = _loadFromStorage()
  books = books.map(book => book.id === bookToUpdate.id ? bookToUpdate : book)
  _saveToStorage(books)
...
function _createBook(title, price = utilService.getRandomIntInclusive(10, 200)) {
  return {
